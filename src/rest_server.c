...
static struct {
	struct MHD_Daemon *post;
	struct MHD_Daemon *get;
	struct MHD_Daemon *del;
} http_daemons;

static void save_device_config(cJSON *json_data) {
...
static enum MHD_Result handle_get_request(
		void *cls,
		struct MHD_Connection *connection,
		const char *url,
		const char *method,
...
int init_http_servers() {
	...
	if (!http_daemons.get) {
		...
		MHD_stop_daemon(http_daemons.post);
		return 1;
	}
	log_info("GET server running on port %d", GET_PORT);

	http_daemons.del = MHD_start_daemon(
	...
	if (!http_daemons.del) {
		...
		MHD_stop_daemon(http_daemons.post);
		MHD_stop_daemon(http_daemons.get);
		return 1;
	}
...
void deinit_http_servers() {
	if (http_daemons.post) {
		MHD_stop_daemon(http_daemons.post);
		http_daemons.post = NULL;
	}
	if (http_daemons.get) {
		MHD_stop_daemon(http_daemons.get);
		http_daemons.get = NULL;
	}
	if (http_daemons.del) {
		MHD_stop_daemon(http_daemons.del);
		http_daemons.del = NULL;
	}
	log_info("HTTP servers stopped.");
}
